...
import type { DeepRequired, IPadding } from "../../interface/Common";
import {
  ControlComponent,
  ControlIndentation,
...
import type { Override } from "../override/Override";
...
export class Draw {
  ...
  public getMargins(): IMargin {
    return this.getOriginalMargins().map(
      (m) => m * this.options.scale,
    ) as IMargin;
  }
  ...
  public getTdPadding(): IPadding {
    ...
    return tdPadding.map((m) => m * scale) as IPadding;
  }
